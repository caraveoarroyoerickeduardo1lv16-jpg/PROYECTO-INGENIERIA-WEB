...
<div class="page">
    ...
    <form method="post" id="formCheckout">
        <!-- ✅ NUEVO: día seleccionado para envío (0=hoy,1=mañana,2=pasado) -->
            ...
            <?php if (!empty($direccionesUsuario)): ?>
                <div class="saved-addresses">
                    ...
                    <?php foreach ($direccionesUsuario as $dir): ?>
                        <div class="saved-address-card" data-dir-id="<?= (int)$dir['id'] ?>">
                            ...
                            <!-- ✅ BOTONES: usar + eliminar (solo UI) -->
                            <div class="saved-form saved-actions">
                                <button type="submit"
                                        name="usar_direccion"
                                        value="<?= (int)$dir['id'] ?>"
                                ...
                                <button type="button"
                                        class="btn-danger btn-sm js-ocultar-direccion">
                                    Eliminar dirección
                                </button>
                            </div>
                        </div>
        ...
        <section id="paso3" class="checkout-container <?= $paso === 3 ? '' : 'hidden' ?>">
            ...
            <div class="slots-group">
                <?php for ($h = 9; $h < 21; $h++): $label = formatHourLabel($h).'-'.formatHourLabel($h+1); ?>
                    <!-- ✅ IMPORTANTE: ya NO marcamos ninguno checked por defecto -->
...
document.addEventListener("click", (e) => {
  if (e.target.classList.contains("js-ocultar-direccion")) {
    const card = e.target.closest(".saved-address-card");
    if (card) card.remove();
  }
});

/* ✅ Si HOY está agotado: deshabilitar botón continuar */
function setContinuar(enabled) {
  const btn = document.getElementById("btnContinuarPago");
  if (!btn) return;
  btn.disabled = !enabled;
}

function syncDiaEnvio(dia){
  const hid = document.getElementById("diaEnvio");
  if (hid) hid.value = String(dia);
}

/* ✅ Hook al filtro de slots (usa lo que ya hace tu checkout.js) */
document.addEventListener("DOMContentLoaded", () => {
  // por defecto hoy
  syncDiaEnvio(0);

  // cada vez que cambian tabs del paso 3, actualizamos dia_envio
  document.querySelectorAll('#paso3 .day-button').forEach(btn=>{
    btn.addEventListener("click", ()=>{
      syncDiaEnvio(parseInt(btn.dataset.dia,10));
      // se evalúa después de que tu JS filtre
      setTimeout(validarSlotsVisibles, 0);
    });
  });
...
function validarSlotsVisibles(){
  const paso3 = document.getElementById("paso3");
  if (!paso3) return;
  const visibles = Array.from(paso3.querySelectorAll(".slot")).filter(s => s.style.display !== "none");
  const hay = visibles.length > 0;

  // si no hay slots visibles, no dejar avanzar
  setContinuar(hay);

  // si hay, forzar selección del primero visible si no hay seleccionado
  if (hay) {
    const checked = paso3.querySelector('input[name="horario"]:checked');
    if (!checked) {
      const firstRadio = visibles[0].querySelector('input[type="radio"]');
      if (firstRadio) {
